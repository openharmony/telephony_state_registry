...
namespace OHOS {
namespace Telephony {
bool TelephonyObserverProxyHolder::InitStateObserverProxy()
{
    if (telephonyStateNotify_ == nullptr) {
        ...
        recipient_ = new ServiceInterfaceDeathRecipient<TelephonyObserverProxyHolder>();
...
void TelephonyObserverProxyHolder::ResetServiceProxy()
{
    std::lock_guard<std::mutex> lock(mutex_);
...
int32_t TelephonyObserverProxyHolder::AddStateObserver(const sptr<TelephonyObserverBroker> &telephonyObserver,
    int32_t subId, uint32_t mask, const std::u16string &callingPackage, bool notifyNow)
{
    if (InitStateObserverProxy()) {
...
int32_t TelephonyObserverProxyHolder::RemoveStateObserver(int32_t subId, uint32_t mask)
{
    if (InitStateObserverProxy()) {
